// Universidad de La Laguna
// Escuela Superior de Ingeniería y Tecnología
// Grado en Ingeniería Informática
// Asignatura: Computabilidad y Algoritmia
// Curso: 2º
// Práctica 1: Símbolos, alfabetos y cadenas
// Autor: Pablo de la Fuente Rodríguez
// Correo: alu0101336152@ull.edu.es
// Fecha: 01/10/2022
// Archivo func_aux.h: Definición de funciones auxiliares.         
// Referencia:
// Historial de revisiones:
//       01/10/2022 - Creación (primera versión) del código.
//       02/10/2022 - Continuación del código.
//       03/10/2022 - Continuación del código.
//       04/10/2022 - Continuación del código.
//       05/10/2022 - Continuación del código.

#ifndef _CyA_P01_FUNC_AUX_  //<PROJECT>_<PATH>_<FILE>_H_
#define _CyA_P01_FUNC_AUX_

#include <string>
#include <sstream>

std::string GetAlpString(std::string);    // Obtiene la parte de la línea del fichero que correspondería al alfabeto.
std::string GetCadString(std::string);    // Obtiene la parte de la línea del fichero que correspondería a la cadena.

#endif